...
rt_size_t touch_gt1151_readpoint(struct rt_touch_device *touch, void *data_buf, rt_size_t touch_num)
{
    ...
    else
    {
        ...
        if (x > GT1151_TOUCH_WIDTH - 1)
            x = GT1151_TOUCH_WIDTH;
        if (y > GT1151_TOUCH_HEIGHT - 1)
            y = GT1151_TOUCH_HEIGHT;
...
rt_err_t gt1151_hw_init(struct gt1151_object *object)
{
    ...
    object->irq_pin = rt_pin_get("PH.7");
    object->rst_pin = rt_pin_get("PI.8");
